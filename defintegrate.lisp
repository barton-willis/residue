...
(defun $billy (e x a b)
  (setq e ($partfrac e x))
  (cond ((and (mplusp e))
          (let ((ans (mapcar #'(lambda (s) (integrate-rational-minf-to-inf ($num s) ($denom s) x)) (cdr e))))
		  	(if (some #'null ans)
				nil
				($expand (fapply 'mplus ans) 1 0))))
	    (t 
		   (integrate-rational-minf-to-inf ($num e) ($denom e) x) 1 0)))
...
;; The function all-residues-rational-fun finds the residues at every pole. We could gain some speed
;; by putting an optional filter on all-residues-rational-fun that only computes the residues at the
;; poles that satisfy a predicate.
(defun integrate-rational-minf-to-inf (p q x)
"Integrates the rational function p/q over the real line from minf to inf using Cauchy's integral
 formula. If a pole lies on the real axis, the integral is interpreted in the principal value sense."
  (let ((pv nil) ($algebraic t))
    (cond
      ((eq t (mgrp (add 1 ($hipow ($expand p) x)) ($hipow ($expand q) x)))
       (diverg))  ; Degree condition: integral diverges
      (t
       (multiple-value-bind (res poles) (all-residues-rational-fun p q x)
         (flet ((residue-weight (p)
           ...
           (let ((sum (fapply 'mplus (mapcar #'(lambda (r p) (mul r (residue-weight p))) res poles))))
...
(defun zmtorat (n d s fn1 ivar)
  (push (ftake 'mlist (div n d) ivar) *larry*)
  (prog (c)
...
(defun log-to-plog (e x)
  (cond (($mapatom e) e)
        ((and (eq (caar e) '%log) (not (freeof x e)))
		    (let ((implicit-real t)) (ftake '%plog (log-to-plog (second e) x))))

		(($subvarp (mop e)) ;subscripted function
		     (subfunmake 
		      (subfunname e) 
			  (mapcar #'(lambda (q) (log-to-plog q x)) (subfunsubs e)) 
			  (mapcar #'(lambda (q) (log-to-plog q x)) (subfunargs e))))
        (t 
		  (fapply (caar e) (mapcar #'(lambda (q) (log-to-plog q x)) (cdr e))))))
...
;; Change integrate(log(xi) * f(x),x,a,b) to integrate(log(g) * f(xxx)
(defun $lal (e xi x a b)
   (let* (($radsubstflag t)
         (g (gensym))
		 ;(*defint-assumptions* nil)
	     (ee (sratsimp (div ($ratsubst g xi e) (sdiff xi x)))))
	(if (freeof x ee)
		(dintlog ee g g (maxima-substitute a x xi) (maxima-substitute b x xi))
		nil)))

(defun defint-exp-subst (e x a b)
  "Apply the substitution `x = exp(g)` to the definite integral `integrate(f(x),x,a,b)`. 
   Often, `f(x)` has the form `log(x) ff(x)`, but this is neither required or checked. 
   Dispatches `defint` on the transformed integrand using the new variable `g`."
  (let* ((g (gensym))
         ($expintrep '$gamma_incomplete)
         (cntx ($supcontext))
         ($logsimp t) ; we want log(exp(g)) to simplify to g
         (exp-g (ftake 'mexpt '$%e g))) ;exp-g = exp(g)

    ;(mtell "reset = ~M ~%" (mfuncall '$reset))
    (unwind-protect
        (progn
	    ;; The function `fn` transforms the limits of integration
	    (flet ((fn (q)
		   (cond ((eql q 0) '$minf)
		         ((eq q '$inf) '$inf)
				 (t (ftake '%log q)))))
		  (let ((aa (fn a)) (bb (fn b))) ;new lower and upper limits of integration 
		    (assume (ftake 'mgreaterp g aa))
            (assume (ftake 'mgreaterp bb g))
			(mtell "facts = ~M ~%" (mfuncall '$facts))
            (defint 
               (mul exp-g (maxima-substitute exp-g x e)) ; new integrand
               g ; new integration variable
               aa ;new lower limit of integration
			   bb)))) ;new upper limit of integration
      ($killcontext cntx))))

(defun defint-baz (e x a b)
  "Apply the substitution `x = exp(g)` to the definite integral `integrate(f(x),x,a,b)`. 
   Often, `f(x)` has the form `log(x) ff(x)`, but this is neither required or checked. 
   Dispatches `defint` on the transformed integrand using the new variable `g`."
  (let* ((g (gensym))
         ...
         (cntx ($supcontext))
         ($logsimp t) ; we want log(exp(g)) to simplify to g
         (exp-g (ftake 'mexpt '$%e (neg g)))) ;exp-g = exp(g)
	    ...
	    (flet ((fn (q)
		  ...
		  (let ((aa (fn a)) (bb (fn b))) ;new lower and upper limits of integration 
            ...
            (defint 
               (mul exp-g (maxima-substitute exp-g x e)) ; new integrand
               g ; new integration variable
               bb ;new lower limit of integration
			   aa)))) ;new upper limit of integration
      ($killcontext cntx))))

(defvar *ftc* 0)
(defun defint-by-ftc (e x lo hi)
  (let ((ee (antideriv e x)))
    ;; antideriv returns nil when Maxima fails to find an antiderivative,
	;; so we need to check that ee is not nil.
    (if (and ee (continuous-p ee x x))
		  (let ((llo ($limit ee x lo '$plus))
		        (hhi ($limit ee x hi '$minus)))
			;; We should have a standard way to do this:
			(cond ((and (eq hhi '$inf) (eq llo '$inf))
			        (diverg))
                  ((and (eq hhi '$minf) (eq llo '$minf))
			       (diverg))
				  ((or (eq hhi '$und) (eq llo '$und))
			       (diverg)) 
				  ((or (eq hhi '$ind) (eq llo '$ind))
				   '$ind)
				  (t ($limit (sub hhi llo)))))
		nil)))
...
(defun definte-error-or-nounform (e x lo hi)
  (mtell (intl:gettext "defint: integral is divergent"))
  (if (and (member e *infinities*) *nodiverg*)
     (throw 'divergent 'divergent)
     (ftake '%integrate e x lo hi)))

(defmfun $mydefint (e x lo hi)
    (let ((*global-defint-assumptions* nil)
	      (*defint-assumptions* nil)
		  (*current-assumptions* nil)
		  (*sin-cos-recur* 0)
		  (*rad-poly-recur* 0)
          (*dintexp-recur* 0)
		  (*dintlog-recur* 0)
  		  (*loopstop* 0))
	  (defint-new e x lo hi)))

(defun defint-by-mbag (e x lo hi)
"If e is an mbag, map q -> integrate(q,x,lo,hi) over e; if not, return nil."
	(and (mbagp e)
	     (fapply (caar e) (mapcar #'(lambda (q) (defint-new q x lo hi)) (cdr e)))))

(defun defint-by-initial-analysis (e x lo hi)
	(let ((*loopstop* 0)) 
			(or (initial-analysis ($expand e 1 0) x lo hi)
			    (initial-analysis e x lo hi))))

(defvar *defint-methods*
   (list 'defint-by-mbag
         'defint-by-symmetric-interval 
		 'defint-log-exp
		 'defint-by-initial-analysis))
	;	 'defint-by-ftc))
  ...
  (let (($logabs nil)
    ...
    (unwind-protect
        (progn
		  ...
		  (when (or (null ans) (member ans *infinities*))
		    (definte-error-or-nounform e x lo hi))
...
(defun log-continuous-p (e x pt)
  (let* ((ee (risplit (first e))) (re (car ee)) (im (cdr ee)))
  (and 
    ;; argument of log must be continuous
    (continuous-p (first e) x pt)
	;; either (i) Re(z) > 0, (ii) Im(z) = 0, or (iii) Im(z) # 0
	(or (eq t (mgrp re 0)) (eq t (meqp im 0)) (eq t (mnqp im 0))))))
(setf (gethash '%log *c-hashtable*) 'log-continuous-p)

(defun gamma-incomplete-continuous-p (e x pt)
  (log-continuous-p (third e) x pt))
(setf (gethash '%gamma_incomplete *c-hashtable*) #'gamma-incomplete-continuous-p)
...
;(defun gamma-incmplete-continuous-p (e x pt)
;	()

(defun expintegral_e-continuous-p (e x pt)
  (or t (continuous-p (second e) x pt)))
(setf (gethash '%expintegral_e *c-hashtable*) 'expintegral_e-continuous-p)
